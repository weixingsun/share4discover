...
function buildGeneralParams(devToken) {
  var params = {
    access_id: Platform.OS === 'ios' ? 111111111 : 2100240971,
...
function buildTagParams(tags,op) {
  var params = {
    access_id: Platform.OS === 'ios' ? 111111111 : 2100240971,
    timestamp: Math.ceil(Date.now() / 1000),
  };

  if (tags) params.tags_list = tags;
  if (op) params.tags_op = op;
...
function pushp2p(devToken, title, content, kv) {
  ...
  params.sign = calcSignature(
    ...
    params, getSecretKey());
  //alert('p2p.push.sign='+params.sign+'\ndev='+devToken+'\ntitle='+title+'\ncontent='+content+'\nkey='+getSecretKey())
  fetch(
    ...
    .then(res => console.log('p2p return:'+JSON.stringify(res.text())))
    .catch(err => console.log('p2p err:'+err));
}
...
function broadcast(title,content) {
  ...
  params.sign = calcSignature(
    ...
    params, getSecretKey());
  //alert('broadcast.push.sign='+params.sign+'\ndev='+devToken+'\ncontent='+content+'\ntitle='+title+'\nkey='+getSecretKey())
  fetch(
...
function pushtags(tags,op,title,content, kv) {
  ...
  params.sign = calcSignature(
    ...
    params, getSecretKey());
  //alert('tags.push.sign='+params.sign+'\ndev='+devToken+'\ncontent='+content+'\ntitle='+title+'\nkey='+getSecretKey())
  fetch(
