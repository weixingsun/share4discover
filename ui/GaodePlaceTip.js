...
const GaodePlaceTip = React.createClass({
  ...
  _request(text) {
    ...
    if (text.length >= this.props.minLength) {
	  ...
	  //console.log(responseJSON)
          if (typeof responseJSON.tips !== 'undefined') {
            if (this.isMounted()) {
              this._results = responseJSON.tips;
              this.setState({
                dataSource: this.state.dataSource.cloneWithRows(this.buildRowsFromResults(responseJSON.tips)),
              });
            }
          }
          if (typeof responseJSON.error_message !== 'undefined') {
      ...
      var url='http://restapi.amap.com/v3/assistant/inputtips?keywords=' + encodeURI(text) + '&' + Qs.stringify(this.props.query);
  ...
  _renderRow(rowData = {}) {
    rowData.description = rowData.name +' '+ rowData.district + ' ' + rowData.address;
...
module.exports = GaodePlaceTip; //{GaodePlaceTip, create};
